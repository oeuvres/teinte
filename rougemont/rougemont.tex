%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% LaTeX model for Rougemont chapters and articles %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Needed before document class
\RequirePackage{pdftexcmds} % needed for tests expressions
\RequirePackage{fix-cm} % correct units

\documentclass[twocolumn]{article}
\usepackage[%
  a4paper,
  inner=15mm,
  outer=15mm,
  top=20mm,
  bottom=18mm,
  marginparsep=0pt,
]{geometry}
\setlength{\columnsep}{10mm}

%%%%%%%%%%
% Colors %
%%%%%%%%%%
% before Teinte macros
\usepackage[dvipsnames]{xcolor}
\definecolor{rubric}{HTML}{cf1408} % the tonic
\colorlet{borderline}{rubric!10!} % definecolor need exact code
\definecolor{shadecolor}{gray}{0.95}

\usepackage[]{roboto} % SmallCaps, Regular is a bit bold
\setmainfont[
  ItalicFont={Roboto Light Italic},
]{Roboto}
\setsansfont{Roboto} % seen, if not set, problem with printer
\newfontfamily\fontrun[]{Roboto Condensed Light} % condensed runing heads
\newfontfamily\fontquote[ItalicFont={Roboto Italic},]{Roboto Light} % light quotes


%%%%%%%%%%%%%%%%%
% Teinte macros %
%%%%%%%%%%%%%%%%%
\input{../latex/teinte}
\setdefaultlanguage[frenchpart=false]{french} % bug on part
\renewcommand{\headrulewidth}{0.1pt}
\renewcommand{\headrule}{{\color{rubric}\hrule}}
\renewcommand{\thefootnote}{\bfseries\textcolor{rubric}{\arabic{footnote}}} % color for footnote marks
\titleformat{name=\section}
  [display]{}{}{}{}
  [\vbox{\color{rubric}\Large\textbf{#1}}]
% \titlespacing{\section}{0pt}{0pt plus 4pt minus 2pt}{\baselineskip}

\fancypagestyle{main}{%
  \fancyhf{}
  \setlength{\headheight}{1.5em}
  \fancyhead{} % reset head
  \fancyfoot{} % reset foot
  \fancyhead[L]{\truncate{0.45\headwidth}{\fontrun\elbibl}} % book ref
  \fancyhead[R]{\truncate{0.45\headwidth}{ \fontrun\nouppercase\eltitle}} % Chapter title
  \fancyhead[C]{\thepage}
}

\renewenvironment{quoteblock}
  {\begin{quoting}\fontquote} %
  {\end{quoting}}


\begin{document}

\thispagestyle{empty}
\twocolumn[{%
  \begin{tikzpicture}[remember picture,overlay]
    \node[xshift=15mm,yshift=-15mm,anchor=north west] at (current page.north west){%
      \includegraphics[width=30mm]{ddr-red.png}
    };
    \node[xshift=15mm,yshift=-6mm,anchor=north west] at (current page.north west){%
      \includegraphics[width=50mm]{rougemont20-noir.png}
    };
  \end{tikzpicture}
  \RaggedLeft\parindent0pt\setlength{\RaggedLeftLeftskip}{35mm}
  \vskip-2em
  {\LARGE \elauthor (\eldate)\par}
  {\LARGE\normalfont \emph{\elbook}\par}
  {\LARGE \href{\elurl}{\eltitle}\footnotemark\par}
  \vskip3em
}]
\footnotetext{\href{\elurl}{\dotuline{\elurl}}}
\pagestyle{main} % after style
\bigskip
\makeatletter\@starttoc{toc}\makeatother % toc without new page
\bigskip



%text%


\end{document}
